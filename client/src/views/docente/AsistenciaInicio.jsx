...
export const AsistenciaInicio = () => {
  ...
  const cargarGrados = async () => {
    const response = await fetch(`http://localhost:4000/horarioD/${user.id}`, {
      headers: {
  ...
  const asignaturasVisibles = asignaturas.slice(
    startIndexC,
  ...
  return (
    <div>
      ...
      <div className="px-5 mx-4 mt-4 border border-gray-300 rounded-lg">
        ...
        <div className="grid grid-cols-1 gap-4 p-4 mx-auto mt-2 sm:grid-cols-3 h-96">
          {asignaturasVisibles.map(({ a_nombre, gid, aid }, index) => (
            <Link key={index} to={`${aid}/${gid}`}>
              <div
                key={index}
                className="overflow-hidden border border-gray-300 rounded-lg h-80 border-opacity-90 hover:border-blue-500 hover:shadow-xl"
              >
                <img
                ...
                <div className="px-2 py-3 transition duration-300 ease-in cursor-pointer select-none">
                  <p className="text-gray-400">{aid}</p>
